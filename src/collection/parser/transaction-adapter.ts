...
import { BalanceChange, convertToUiAmount, ParseConfig, PoolEventType, SolanaTransaction, TokenInfo, TransactionStatus } from '../../type/index';
import { getInstructionData, getProgramName, getPubkeyString } from '../../lib/utils';
...
export class TransactionAdapter {
  ...
  constructor(
    private tx: MapTransactionEncodingToReturnType[4],
   ...
   */
  get slot() {
    return toSafeNumber(this.tx.slot);
  }
  ...
  get signers(): string[] {
    ...
    if (message instanceof MessageV0 || 'header' in message) {
      const numRequiredSignatures = message.accountKeys.filter((key: any) => key.signer).length || 1
  ...
  get computeUnits(): number {
    return toSafeNumber(this.tx.meta?.computeUnitsConsumed, 0);
  }
  ...
  extractAccountKeys() {
    if (this.isMessageV0) {
      ...
      const key2 = this.txMessage.accountKeys.filter((key: any) => !key.signer).map((it: any) => getPubkeyString(it)) || [];
      const key3 = this.txMessage.accountKeys.filter((key: any) => key.signer).map((it: any) => getPubkeyString(it)) || [];
      return [...keys, ...key2, ...key3];
    } else if (this.version == 0) {
      // parsed transaction
      const keys = this.getAccountKeys(this.txMessage.accountKeys) || [];
      const key2 = this.getAccountKeys(this.txMessage.accountKeys.filter((key: any) => !key.signer).map((it: any) => it.pubkey)) || [];
      const key3 = this.getAccountKeys(this.txMessage.accountKeys.filter((key: any) => key.signer).map((it: any) => it.pubkey)) || [];
      return [...keys, ...key2, ...key3];
    } else {
